...
namespace View._4BI.Movimentacao
{
    public partial class frmReserva : Form
    {
        ...
        private void btnInserir_Click(object sender, EventArgs e)
        {
            ...
            CReserva.CReservaClient oProxy = new CReserva.CReservaClient();
            oProxy.Open();

            try
            {
                oProxy.Reservar(aluno, livro);

                MessageBox.Show("Reserva realizada com sucesso!", "Confirmação", MessageBoxButtons.OK, MessageBoxIcon.Information);

                CReserva.CReservaClient oProxyR = new CReserva.CReservaClient();
                oProxyR.Open();



                var listaReserva = oProxyR.listaTodasReservasAluno(aluno);
                int i = 0;
                foreach (var item in listaReserva)
                {
                    var Al = listaReserva[i].ID_ALUNO;
                    var Li = listaReserva[i].ID_LIVRO;
                    ...
                    dtgEmprestimos.Rows.Add(listaReserva[i].ID_RESERVA, oLi.TITULO, listaReserva[i].DATA_RESERVA, oAlu.NOME);
